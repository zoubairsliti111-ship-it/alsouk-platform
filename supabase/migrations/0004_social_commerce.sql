...
-- ---------------------------------------------------------------------------
-- Social-commerce profile fields on companies (additive, nullable). These
-- power the profile header/about without a parallel table. companies stays the
-- single source of truth. Existing rows/data are untouched.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- API role privileges. Supabase's grant-then-RLS model: the API roles get
-- table privileges and RLS decides which rows they may touch. Hosted Supabase
-- grants these automatically for new tables; declaring them here keeps the
-- schema portable (self-hosted / `supabase db reset`) and is idempotent.
-- Applied at the end of this file (see GRANTs section) once tables exist.
-- ---------------------------------------------------------------------------

-- ---------------------------------------------------------------------------
-- company_members — team access for a company.
-- ---------------------------------------------------------------------------
...
drop policy if exists "Members readable by the team" on public.company_members;
create policy "Members readable by the team"
  on public.company_members for select using (public.is_company_member(company_id));
